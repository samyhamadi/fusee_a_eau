...
void LSM9DS0::initAccel()
{
    ...
    xmWriteByte(CTRL_REG2_XM, 0x20); // Set scale to 16g, Anti-alias filter bandwidth = 50 Hz
...
void LSM9DS0::readAccel()
{
  ...
  //Get x
    Temp= OUT_X_L_A | 0x80;
    i2c_->i2c.write(xmAddress <<1,&Temp,1);
    i2c_->i2c.read( xmAddress <<1, (char*)&ax, 2);
...
//   Temp |= xmReadByte(OUT_Y_L_A);
 // ay = Temp;
  //   i2c_->readBytes( xmAddress <<1, OUT_Y_L_A, 2, (uint8_t *) &ay);
    i2c_->i2c.read( xmAddress <<1, (char*)&ay, 2);
  //Get z
//   Temp=0;
...
//   az = Temp;
 //  i2c_->readBytes( xmAddress <<1, OUT_Z_L_A, 2, (uint8_t *) &az);
    i2c_->i2c.read( xmAddress <<1, (char*)&az, 2);
}
...
{   
  ...
  gx = Temp;
    // Temp= OUT_X_L_G | 0x80;
    // i2c_->i2c.write(gAddress <<1,&Temp,1);
    // i2c_->i2c.read( gAddress <<1, (char*)&gx, 2);
  ...
  gy = Temp;
    //   i2c_->i2c.read( gAddress <<1, (char*)&gy, 2);
  //Get z
  Temp=0;
  ...
  gz = Temp;
    // i2c_->i2c.read( gAddress <<1, (char*)&gz, 2);
}
